 ...
 */
export const ATLAS_FIXTURE_PATH =
  "docs/vnext/fixtures/atlas-project.draft.yaml" as const;

function atlasFixtureUrl(): URL {
  return new URL(`../../../${ATLAS_FIXTURE_PATH}`, import.meta.url);
}

/** Read the Atlas fixture as raw YAML text. */
export async function readAtlasFixtureText(): Promise<string> {
  return readFile(atlasFixtureUrl(), "utf8");
}

/** Read, parse, and fully validate the Atlas fixture. Throws on any issue. */
export async function loadAtlasFixture(): Promise<ProjectContract> {

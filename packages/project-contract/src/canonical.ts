...
function canonicalizeAt(value: unknown, path: string): CanonicalValue {
  ...
  if (kind === "number") {
    ...
    if (!Number.isFinite(numeric)) {
      throw new CanonicalizationError(
        `non-finite number ${String(numeric)} cannot be canonicalized`,
        path,
      );
    }
  ...
  if (Array.isArray(value)) {
    return value.map((entry, index) =>
      canonicalizeAt(entry, `${path}[${index}]`),
    );
  }
  ...
  throw new CanonicalizationError(
    `values of type ${kind} cannot appear in a portable contract`,
    path,
  );
}

...
function fromZod(error: z.ZodError, document: unknown): ContractIssue[] {
  return error.issues.flatMap((entry): ContractIssue[] => {
    ...
    if (entry.code === "unrecognized_keys") {
      return entry.keys.map((key) =>
        issue(
          "unknown_field",
          path === "" ? key : `${path}.${key}`,
          `unknown field "${key}"; the draft schema rejects unrecognized fields so that typos and invented fields surface immediately`,
          { field: key },
        ),
      );
    }
    ...
    if (
      ...
      return [
        issue(
          "missing_field",
          path,
          `required field is missing; expected ${entry.expected}`,
 ...
 * format needs.
 */
export function parseYamlDocument(text: string): ContractResult<unknown> {
  try {
    return ok(loadYaml(text, { schema: JSON_SCHEMA }) as unknown);
  } catch (error) {
    if (error instanceof YAMLException) {
      const mark = error.mark as { line?: number; column?: number } | undefined;
      const where =
        mark === undefined || mark.line === undefined
          ? ""
...
export function parseJsonDocument(text: string): ContractResult<unknown> {
  try {
...
export function validateProjectContract(
  ...
  if (
    ...
    return fail([
      issue(
        ...
        "<document>",
        "a project contract must be a mapping at the top level",
      ),
    ]);
  }
  ...
  for (const finding of findSecrets(contract)) {
    issues.push(
      issue("secret_value_detected", finding.path, finding.explanation, {
...
function detectFormat(text: string, hint?: ContractFormat): ContractFormat {
  if (hint !== undefined) {
    return hint;
  }
...
export function parseProjectContract(
  ...
  const document =
    chosen === "json" ? parseJsonDocument(text) : parseYamlDocument(text);
  if (!document.ok) {

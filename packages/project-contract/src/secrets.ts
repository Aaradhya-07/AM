...
const CREDENTIAL_CONTEXT =
  /(?:password|passwd|pwd|secret|token|api[_-]?key|apikey|credential|private[_-]?key|access[_-]?key|client[_-]?secret|auth)/i;
 ...
 */
function looksLikePathOrLocator(value: string): boolean {
  return (
    /^[.@\w-]+(?:[/\\][.@\w-]+)+$/.test(value) ||
...
const NOT_A_SECRET: SecretVerdict = {
  isSecret: false,
  detector: "none",
 ...
 */
export function classifyValue(value: string, path: string): SecretVerdict {
  ...
  if (CREDENTIAL_CONTEXT.test(path) && opaqueToken) {
    return {
      ...
      explanation:
        "a high-entropy value in a credential-bearing field looks like a literal credential; store a reference instead",
    };
  }
...
export function findSecrets(root: unknown, basePath = ""): SecretFinding[] {

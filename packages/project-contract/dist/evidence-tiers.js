...
export function tierMeets(tier, floor) {
...
export function tierForEvidenceKind(kind) {
    return KIND_TIER[kind];
}
export function tierForEvidence(record) {
 ...
 */
export const RATIFIED_EVIDENCE_FLOORS = {
 ...
 */
export const ADMISSIBLE_EVIDENCE_KINDS = {
 ...
 * something else.
 */
const CANDIDATE_SCOPED_SUBJECTS = new Set([
 ...
 */
const WORKLOAD_SCOPED_SUBJECTS = new Set([
    "workload_quality_hard_gate",
    "latency_throughput_hard_gate",
    "token_hard_gate",
]);
/** Subjects that can only be settled against the candidate's real hardware. */
const HARDWARE_SCOPED_SUBJECTS = new Set([
    "hardware_compatibility_estimate",
    "hardware_performance_gate",
]);
const ADMITTED = { admitted: true, reason: "" };
/**
 ...
 */
export function admitEvidence(record, constraint, context) {
    ...
    if (!appliesToConstraints.includes(constraint.id)) {
        return reject(appliesToConstraints.length === 0
    ...
    if (!admissibleKinds.includes(record.kind)) {
        return reject(`evidence of kind "${record.kind}" cannot establish ${floorSubject}; that subject requires ${admissibleKinds.join(" or ")}`);
    }
    ...
    if (CANDIDATE_SCOPED_SUBJECTS.has(floorSubject)) {
        ...
        if (candidateRef === undefined || candidateRef === null) {
            return reject(`${floorSubject} is candidate-specific, but no candidate context was supplied; missing context is insufficient evidence, not permission to skip the check`);
        }
        if (appliesTo.candidate_ref === null) {
            return reject(`${floorSubject} is candidate-specific, but this evidence is not attributed to any candidate`);
        }
        if (appliesTo.candidate_ref !== candidateRef) {
            return reject(`this evidence describes candidate "${appliesTo.candidate_ref}", not "${candidateRef}"`);
        }
        if (record.kind === "measured_evaluation" &&
            record.value.candidate_ref !== candidateRef) {
            return reject(`the evaluation was run against candidate "${record.value.candidate_ref}", not "${candidateRef}"`);
        }
    }
    ...
    if (WORKLOAD_SCOPED_SUBJECTS.has(floorSubject)) {
        ...
        if (workloadRef === undefined || workloadRef === null) {
            return reject(`${floorSubject} is workload-specific, but no workload context was supplied`);
        }
        if (appliesTo.workload_ref !== null &&
            appliesTo.workload_ref !== workloadRef) {
            return reject(`this evidence describes workload "${appliesTo.workload_ref}", not "${workloadRef}"`);
        }
    }
    else if (appliesTo.workload_ref !== null &&
        supplied.workloadRef !== undefined &&
        supplied.workloadRef !== null &&
        appliesTo.workload_ref !== supplied.workloadRef) {
        return reject(`this evidence describes workload "${appliesTo.workload_ref}", not "${supplied.workloadRef}"`);
    }
    // --- hardware scope ------------------------------------------------------
    //
    ...
    if (HARDWARE_SCOPED_SUBJECTS.has(floorSubject)) {
        ...
        if (declared === undefined) {
            return reject(`${floorSubject} needs the declared-target hardware context, and none was supplied`);
        }
        const deployment = supplied.deploymentHardwareRef;
        if (deployment === undefined || deployment === null) {
            return reject(`${floorSubject} needs the candidate's deployment hardware, and none was supplied`);
        }
        if (!declared.includes(deployment)) {
            return reject(`deployment hardware "${deployment}" is not a user-declared target, so nothing measured on it can settle a target gate`);
        }
        if (record.kind === "tool_observation") {
            const { target_hardware_ref: target, detected_hardware_ref: detected } = record.value;
            if (target === null || detected === null) {
                return reject("a hardware observation must name both the target it describes and the machine it inspected");
            }
            if (target !== detected) {
                return reject(`declared target "${target}" and detected hardware "${detected}" differ, so this observation is not evidence about the target`);
            }
            if (target !== deployment) {
                return reject(`this observation describes hardware "${target}", not the candidate's deployment target "${deployment}"`);
            }
        }
            ...
            return reject(`this measurement ran on hardware "${appliesTo.hardware_ref}", not the candidate's deployment target "${deployment}"`);
        }
    }
    ...
    if (isHardwareIdentityMismatch(record)) {
        return reject("declared target hardware and detected hardware differ, so this observation is not evidence about the target");
    }
    return ADMITTED;
}
/**
 ...
 */
export function effectiveFloor(policy, subject) {
 ...
 */
export function floorSubjectForConstraint(constraint) {
    ...
    switch (constraint.domain) {
        ...
        case "operability":
            return null;
        default: {
            const exhaustive = constraint.domain;
            return exhaustive;
        }
    }
}
 ...
 */
export function isHardwareIdentityMismatch(record) {
    if (record.kind !== "tool_observation") {
        return false;
    }
 ...
 */
export function assessEvidence(constraint, citedEvidence, policy, context) {
    const supplied = context ?? {};
    const floorSubject = floorSubjectForConstraint(constraint);
    const floor = floorSubject === null ? null : effectiveFloor(policy, floorSubject);
    const excluded = [];
    let bestTier = "T0";
    ...
    if (meetsFloor && floorSubject === "projected_cost_comparison") {
        ...
        if (supplied.hasExplicitCostCalculation !== true) {
            missing.push("a recorded projected monthly cost and the arithmetic behind it");
        }
        if (supplied.hasDeclaredUsageInputs !== true) {
            missing.push("declared-or-stronger workload usage inputs with their assumptions written down");
        }
        if (missing.length > 0) {
            meetsFloor = false;
 ...
 */
export function evaluateConstraint(constraint, assertedStatus, citedEvidence, policy, context) {
    const assessment = assessEvidence(constraint, citedEvidence, policy, context ?? {});
    if (constraint.severity === "informational") {
        return {
            outcome: "not_applicable",
            assessment,
            explanation: "informational constraints are recorded but not scored, so they are never a gate",
        };
    }
    if (assertedStatus === "not_applicable" || assertedStatus === "fail") {
        return {
            ...
            assessment,
            explanation: assertedStatus === "fail"
    ...
    if (!assessment.meetsFloor) {
        const excludedNote = assessment.excluded.length === 0
            ? ""
        ...
        return {
            ...
            assessment,
            downgradedFrom: "pass",
            explanation: `${assessment.shortfall ?? "the evidence floor was not met"}${excludedNote}; ` +

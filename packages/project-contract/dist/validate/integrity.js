...
import { RATIFIED_EVIDENCE_FLOORS, evaluateConstraint, isHardwareIdentityMismatch, tierRank, } from "../evidence-tiers.js";
import { resolveDecision } from "../resolve.js";
import { RATIFIED_DEFAULT_DENY } from "../schema/remote.js";
function buildIndex(contract) {
    const issues = [];
    const tables = {
        constraint: new Map(),
        workload: new Map(),
    ...
    const collect = (type, path, entries) => {
        entries.forEach((entry, position) => {
            const table = tables[type];
            if (table.has(entry.id)) {
                issues.push(issue("duplicate_id", `${path}[${position}].id`, `duplicate ${type} id "${entry.id}"; ids must be unique within their collection`, { id: entry.id, subject_type: type }));
                return;
            }
    ...
    collect("architecture_node", "architecture.nodes", contract.architecture.nodes);
    collect("architecture_relationship", "architecture.relationships", contract.architecture.relationships);
    collect("repository_binding", "repository_bindings", contract.repository_bindings);
    ...
    const dataClassifications = new Set(contract.workloads.map((workload) => workload.input_classification));
    return { index: { tables, global, dataClassifications }, issues };
}
function checkRef(index, check) {
    ...
    if (foundAs !== undefined && foundAs.length > 0) {
        return issue("reference_wrong_type", check.path, `${check.label} "${check.value}" resolves to a ${foundAs.join(" and ")}, but a ${check.expected} is required`, { reference: check.value, expected: check.expected, found_as: foundAs });
    }
    return issue("reference_not_found", check.path, `${check.label} "${check.value}" does not resolve to any declared ${check.expected}`, { reference: check.value, expected: check.expected });
}
/**
...
function checkIntegrityInner(contract, index) {
    const issues = [];
    ...
    const evidenceById = index.tables.evidence;
    // --- project ---------------------------------------------------------
    const seenDomains = new Set();
    contract.project.priority_order.forEach((domain, position) => {
        if (seenDomains.has(domain)) {
            issues.push(issue("duplicate_id", `project.priority_order[${position}]`, `domain "${domain}" appears more than once in priority_order; the ordering must be unambiguous`, { domain }));
        }
        seenDomains.add(domain);
    });
    // --- evidence policy floors -----------------------------------------
    for (const [subject, declared] of Object.entries(contract.evidence_policy.floors)) {
        if (declared === undefined) {
            continue;
        }
        const ratified = RATIFIED_EVIDENCE_FLOORS[subject];
        if (tierRank(declared) < tierRank(ratified)) {
            issues.push(issue("evidence_floor_below_ratified_baseline", `evidence_policy.floors.${subject}`, `declared floor ${declared} is weaker than the ratified baseline ${ratified} for "${subject}"; floors may be raised but never lowered`, { subject, declared, ratified }));
        }
    }
    // --- constraints ------------------------------------------------------
    ...
    contract.constraints.forEach((constraint, position) => {
        ...
        if (head === "workload" && second !== undefined) {
            if (!index.tables.workload.has(second)) {
                issues.push(issue("constraint_subject_unresolved", `${base}.subject`, `subject "${constraint.subject}" names workload "${second}", which is not declared`, { subject: constraint.subject, workload: second }));
            }
        }
        if (head === "data" && second !== undefined) {
            if (!index.dataClassifications.has(second)) {
                issues.push(issue("constraint_subject_unresolved", `${base}.subject`, `subject "${constraint.subject}" names data classification "${second}", which no workload declares as an input`, { subject: constraint.subject, data_classification: second }));
            }
        }
        constraint.exceptions.forEach((exception, exceptionPosition) => {
            ...
            if (exceptionIds.has(exception.id)) {
                issues.push(issue("duplicate_id", `${exceptionPath}.id`, `duplicate exception id "${exception.id}"`, { id: exception.id }));
            }
    ...
    contract.evidence_refs.forEach((record, position) => {
        ...
        record.applies_to.constraint_refs.forEach((ref, refPosition) => {
            ...
            if (seenConstraintRefs.has(ref)) {
                issues.push(issue("duplicate_id", refPath, `constraint "${ref}" is listed more than once; each supported claim is named exactly once`, { constraint: ref }));
                return;
            }
        ...
        if (record.kind === "measured_evaluation") {
            ...
            if (problem === null &&
                ...
                issues.push(issue("reference_wrong_type", `${base}.value.candidate_ref`, `this evaluation was run against candidate "${record.value.candidate_ref}" but claims to apply to "${record.applies_to.candidate_ref}"`, {
    ...
    contract.candidates.forEach((candidate, position) => {
        ...
        if (candidate.deployment.mode === "local" &&
        ...
        for (const [field, allowedKinds] of Object.entries(MEASUREMENT_KINDS)) {
            const ref = candidate.measurements[field];
            ...
            if (!allowedKinds.includes(record.kind)) {
                issues.push(issue("estimate_presented_as_measurement", path, `${field} cites evidence "${ref}" of kind "${record.kind}", which is not a measurement; expected one of ${allowedKinds.join(", ")}`, { evidence: ref, kind: record.kind, allowed: allowedKinds }));
            }
            ...
            const describes = record.applies_to.candidate_ref ??
                (record.kind === "measured_evaluation"
                    ? record.value.candidate_ref
                    : null);
            if (describes === null) {
                issues.push(issue("evidence_not_applicable", path, `${field} cites evidence "${ref}" that is not attributed to any candidate; a measurement must name the candidate it describes`, { evidence: ref, candidate: candidate.id }));
            }
            else if (describes !== candidate.id) {
                issues.push(issue("evidence_not_applicable", path, `${field} cites evidence "${ref}", which describes candidate "${describes}", not "${candidate.id}"`, { evidence: ref, describes, candidate: candidate.id }));
            }
            if (isHardwareIdentityMismatch(record)) {
                issues.push(issue("estimate_presented_as_measurement", path, `${field} cites hardware evidence "${ref}" whose declared target and detected hardware differ; a mismatched observation cannot describe the target`, {
                    evidence: ref,
                    target_hardware_ref: record.kind === "tool_observation"
                        ? record.value.target_hardware_ref
                        : null,
                    detected_hardware_ref: record.kind === "tool_observation"
        ...
        candidate.constraint_results.forEach((result, resultPosition) => {
            ...
            result.evidence_refs.forEach((ref, refPosition) => {
                ...
                if (!record.applies_to.constraint_refs.includes(result.constraint_ref)) {
                    issues.push(issue("evidence_not_applicable", `${resultPath}.evidence_refs[${refPosition}]`, record.applies_to.constraint_refs.length === 0
            ...
            const servingWorkload = candidate.workload_ref === null
                ? undefined
                : index.tables.workload.get(candidate.workload_ref);
            ...
            const usageIsUserEstablished = usageBasis === "user_assumption" || usageBasis === "measured";
            const assumptionsRecorded = usageBasis === "measured" || candidate.estimates.assumptions.length > 0;
            const context = {
                candidateRef: candidate.id,
                workloadRef: candidate.workload_ref,
                declaredTargetHardwareRefs: declaredTargetHardware,
                deploymentHardwareRef: candidate.deployment.mode === "local"
                    ? candidate.deployment.hardware_ref
                    : null,
                hasExplicitCostCalculation: candidate.estimates.monthly_effective_cost_usd !== null &&
                    candidate.estimates.basis !== null,
                hasDeclaredUsageInputs: usageIsUserEstablished && assumptionsRecorded,
            };
            const evaluation = evaluateConstraint(constraint, "pass", cited, contract.evidence_policy, context);
            ...
            if (evaluation.outcome === "unknown") {
                issues.push(issue("pass_below_evidence_floor", `${resultPath}.status`, `this result asserts "pass" for constraint "${result.constraint_ref}" but ${evaluation.explanation}`, {
                    constraint: result.constraint_ref,
    ...
    contract.decisions.forEach((decision, position) => {
        ...
        const checkSelection = (ref, path) => {
            ...
            if (candidate.workload_ref !== scopedWorkload) {
                issues.push(issue("candidate_workload_mismatch", path, `candidate "${ref}" serves ${candidate.workload_ref === null ? "no workload" : `workload "${candidate.workload_ref}"`}, but this decision is scoped to workload "${scopedWorkload}"`, {
                    candidate: ref,
        ...
        if (decision.selected_candidate_ref !== null) {
            checkSelection(decision.selected_candidate_ref, `${base}.selected_candidate_ref`);
        }
        ...
        if (decision.status === "approved") {
            if (!decision.rationale.reviewed_by_user) {
                issues.push(issue("decision_scope_invalid", `${base}.rationale.reviewed_by_user`, `decision "${decision.id}" is approved but was never marked reviewed by a user; an adapter cannot approve on the user's behalf`, { decision: decision.id }));
            }
            // An approval of an older revision is history, not authorisation for
            // the revision now in the contract.
            const coveringApproval = contract.approvals.find((approval) => approval.decision_ref === decision.id &&
                approval.decision_revision === decision.revision);
            if (coveringApproval === undefined) {
                issues.push(issue("approval_content_unresolved", `${base}.status`, `decision "${decision.id}" revision ${decision.revision} is approved but no approval record covers that revision`, { decision: decision.id, revision: decision.revision }));
            }
            if (decision.selected_candidate_ref !== null) {
                ...
                if (candidate !== undefined &&
                    ...
                    issues.push(issue("model_version_unpinned", `${base}.selected_candidate_ref`, `approved decision "${decision.id}" selects a candidate whose model version is floating; an approved decision must reference an exact candidate revision`, {
    ...
    contract.architecture.relationships.forEach((relationship, position) => {
        ...
        if (relationship.data_classification !== null &&
            !index.dataClassifications.has(relationship.data_classification)) {
            issues.push(issue("reference_not_found", `${base}.data_classification`, `data classification "${relationship.data_classification}" is not declared as the input of any workload`, { data_classification: relationship.data_classification }));
        }
    });
    ...
    contract.conformance_rules.forEach((rule, position) => {
        ...
        if (rule.kind === "forbid_dataflow") {
            ...
            if (!index.dataClassifications.has(rule.from.data_classification)) {
                issues.push(issue("reference_not_found", `${base}.from.data_classification`, `data classification "${rule.from.data_classification}" is not declared as the input of any workload`, { data_classification: rule.from.data_classification }));
            }
            if (rule.unless !== null) {
        ...
        rule.allowed_candidate_refs.forEach((ref, refPosition) => {
            ...
            if (candidate !== undefined &&
                ...
                issues.push(issue("candidate_workload_mismatch", path, `candidate "${ref}" serves ${candidate.workload_ref === null ? "no workload" : `workload "${candidate.workload_ref}"`}, but this rule governs workload "${rule.workload_ref}"`, {
                    candidate: ref,
                    candidate_workload: candidate.workload_ref,
    ...
    policy.default_allow.forEach((field, position) => {
        if (denySet.has(field)) {
            issues.push(issue("remote_policy_conflict", `remote_intelligence_policy.default_allow[${position}]`, `"${field}" appears in both default_allow and default_deny; the projection would be ambiguous`, { field }));
        }
        if (RATIFIED_DEFAULT_DENY.includes(field)) {
            issues.push(issue("remote_policy_denied_field_allowed", `remote_intelligence_policy.default_allow[${position}]`, `"${field}" is default-denied by the ratified projection policy and must not be allowed to remote intelligence; hardware capacity may be shared, machine identity may not`, { field }));
        }
    });
    // --- approvals --------------------------------------------------------
    //
    ...
    const decisionsById = index.tables.decision;
    ...
    contract.approvals.forEach((approval, position) => {
        ...
        if (previousApprovedAt !== null && approvedAt < previousApprovedAt) {
            issues.push(issue("approval_history_not_append_only", `${base}.approved_at`, "approval history must be append-only and ordered oldest first; this entry predates the one before it", { approved_at: approval.approved_at }));
        }
        ...
        if (!resolved.ok) {
            ...
            issues.push(issue("approval_content_unresolved", `${base}.content_hash`, `this approval covers the current revision of decision "${decision.id}", but its content cannot be resolved: ${first === undefined ? "resolution failed" : first.message}`, { decision: decision.id, revision: approval.decision_revision }));
            return;
        }
        const expected = hashApprovalContent(resolved.value.content);
        if (expected !== approval.content_hash) {
            issues.push(issue("approval_hash_mismatch", `${base}.content_hash`, `the resolved content of decision "${decision.id}" revision ${decision.revision} no longer matches this approval; approve the new content or restore what was approved`, {
                decision: decision.id,
 ...
 */
export function checkIntegrity(contract) {
